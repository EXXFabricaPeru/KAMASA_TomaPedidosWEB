...
namespace TomaPedidosApi.PDF
{
    public class Cotizacion
    {
        ...
        public byte[] Reporte(DTOPedidoVentaCab Cotizacion)
        {
            byte[] data = Document.Create(document =>
            {
                ...
                document.Page(page =>
                {
                    ...
                    page.Header().ShowOnce().Row(row =>
                    {
                        ...
                        //{
                        //    col.Item().AlignCenter().Text("COTIZACION").Bold().FontSize(12);
                        //});

                        row.RelativeItem().Column(col =>
                        {
                            col.Item().AlignCenter().Text(Cotizacion.Compania).Bold().FontSize(14);
                            col.Item().AlignCenter().Text(Cotizacion.CompaniaDir).FontSize(9);
                            col.Item().AlignCenter().Text(Cotizacion.CompaniaTel).FontSize(9);
                            col.Item().AlignCenter().Text(Cotizacion.CompaniaMail).FontSize(9);
                        ...
                        row.RelativeItem().Column(col =>
                        {
                            ...
                            .AlignCenter().Text($"RUC {Cotizacion.CompaniaRuc}");

                            col.Item().Background("#257272").Border(1)
                            .BorderColor("#257272").AlignCenter()
                            .Text("COTIZACIÓN").FontColor("#fff");

                            col.Item().Border(1).BorderColor("#257272").
                            AlignCenter().Text(Cotizacion.NroPedido);
                    ...
                    page.Content().PaddingVertical(10).Column(col1 =>
                    {
                        col1.Item().Column(col2 =>
                        {
                            ...
                            col2.Item().Text(txt =>
                            {
                                ...
                                txt.Span(Cotizacion.NomCliente).FontSize(10);
                            });

                            col2.Item().Text(txt =>
                            {
                                txt.Span("R.U.C: ").SemiBold().FontSize(10);
                                txt.Span(Cotizacion.RucCliente).FontSize(10);
                            });

                            col2.Item().Text(txt =>
                            {
                                txt.Span("Direccion: ").SemiBold().FontSize(10);
                                txt.Span(Cotizacion.Direccion).FontSize(10);
                            });
                        });
                        ...
                        col1.Item().Table(tabla =>
                        {
                            ...
                            foreach (DTOPedidoVentaDet item in Cotizacion.ListaDetalle)
                            {
